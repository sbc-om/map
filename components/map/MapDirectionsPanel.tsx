...
interface MapDirectionsPanelProps {
  isOpen: boolean;
  onClose: () => void;
  /** Pre-fill destination when right-clicking a point */
  initialTo?: string;
}
 ...
 * - Real route drawn on map (blue polyline + colored pins)
...
export function MapDirectionsPanel({
  ...
  const { result, isLoading, error, getRoute, clearRoute } = useDirections();
  ...
  return (
    <div className="absolute top-0 left-0 h-full w-[360px] bg-white dark:bg-gray-900 shadow-2xl z-[1000] flex flex-col animate-in slide-in-from-left-4 duration-300">
      {/* ── Header ── */}
      <div className="flex items-center gap-3 px-4 py-4 border-b dark:border-gray-800 flex-shrink-0">
        <button
          onClick={handleClose}
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          ...
          <div className="px-4 py-4 bg-blue-50 dark:bg-blue-950/30 border-b dark:border-gray-800">
            ...
            <div className="mt-3 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
              <span className="font-medium text-gray-700 dark:text-gray-300 max-w-[130px] truncate">
                {result.from.name}
              </span>
